...
func _unhandled_input(event):
	if(event is InputEventMouseMotion):
		# increases or decreases the players position along the x axis based on mouse movement
...
func _on_animated_sprite_2d_animation_finished():
	animated_sprite.play("idle") # play idle animation whenever another animation is finished
